...
- [006-emergency-ingredient-swap.md](006-emergency-ingredient-swap.md) — Batch-level ingredient substitution and on-the-fly tweaks when pantry or store can't match the plan. Auto-applies the swap, renders the updated recipe card with a delta line at the bottom, rebalances macros openly using pantry staples only, leaves precisely-bought ingredients and the library recipe untouched. Serves JTBD C1.
