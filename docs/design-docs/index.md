...
| [006-emergency-ingredient-swap.md](./006-emergency-ingredient-swap.md) | accepted | Batch-level ingredient substitution when pantry or store can't match the plan. Auto-applies unambiguous, non-structural swaps; previews ambiguous / structural / hedged ones. Renders the updated cook card with a delta block; rebalances macros openly via pantry staples; leaves precisely-bought ingredients and library recipes untouched. Live-projects to the shopping list. Breakfast swaps work identically via a `'breakfast'` sentinel target. Serves JTBD C1. |
