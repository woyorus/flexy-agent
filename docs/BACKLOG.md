...
- **Agentic plan mutations**: All plan changes are handled by a reasoning LLM re-proposer — no intent classification, no deterministic handlers, no fast/slow split. User says what they want (voice or text), the LLM re-arranges the plan. Post-proposal keyboard shows LLM-generated contextual quick-fix suggestions (e.g., "Move flex to Sunday", "Extend tagine to cover Wednesday") — prioritized for this specific plan, not generic static buttons. The current intent classification layer (flex_move, recipe_swap, etc.) and deterministic mutation handlers are removed. This is the natural evolution of plan 018's re-proposer: v0.0.4 introduces it within the planning flow, v0.0.5 makes it accessible from anywhere via the freeform conversation layer.
