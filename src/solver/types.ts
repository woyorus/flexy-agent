 ...
 * Contains recipe assignments, flex slot suggestions, events, and (deprecated) recipe gaps.
 * The solver runs on this to compute exact calorie targets.
 *
 * Plan 024: the proposal is the single source of truth for the plan arrangement —
 * batches, flex slots, and events together cover every (day, mealType) in the horizon.
 */
export interface PlanProposal {
  ...
  flexSlots: FlexSlot[];
  /**
   * Events (meals out, social meals, etc.) included in the plan.
   * Plan 024: the proposal becomes the single source of truth for events.
 ...
 *
 * Plan 024: days need NOT be consecutive. Events and flex meals in the middle are fine.
 * Days must be in ascending ISO order. The hard constraint is fridge-life:
 * calendarSpan(days[0], lastEatingDay) ≤ recipe.storage.fridgeDays.
 */
export interface ProposedBatch {
  recipeSlug: string;
  recipeName: string;
  mealType: 'lunch' | 'dinner';
  /** ISO dates this batch covers (in-horizon only — the solver sees these). Need not be consecutive. */
  days: string[];
  servings: number;
  /**
